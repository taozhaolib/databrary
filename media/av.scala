...
object AV {
  ...
  def frame(infile : File, offset : Offset, size : Int) : Array[Byte] =
    _frame(infile.getPath, offset.seconds, null, size, size)
  def frame(infile : File, offset : Offset, size : Int, outfile : File) : Unit =
    _frame(infile.getPath, offset.seconds, outfile.getPath, size, size)
