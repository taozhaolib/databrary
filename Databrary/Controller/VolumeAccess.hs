...
viewVolumeAccess vi at@(VolumeAccessTarget ap) = action GET (toRoute vi ++ toRoute at) $
  withVolume PermissionADMIN vi $ \v -> do
...
postVolumeAccess api vi at@(VolumeAccessTarget ap) = action POST (api, vi, at) $
  withVolume PermissionADMIN vi $ \v -> do
    a <- maybeAction =<< lookupVolumeAccessParty v ap
    u <- peek
