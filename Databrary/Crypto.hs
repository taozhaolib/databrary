...
signature :: ResourceM m => BS.ByteString -> m BS.ByteString
signature msg = do
  secret <- getResource resourceSecret
  return $ hmac secret msg

sign :: ResourceM m => BS.ByteString -> m BS.ByteString
sign msg = do
  sig <- signature msg
  return $ sig `BS.append` msg

unSign :: ResourceM m => BS.ByteString -> m (Maybe BS.ByteString)
unSign sigmsg = do
  sig' <- signature msg
  return $ do
    guard (sig == sig')
