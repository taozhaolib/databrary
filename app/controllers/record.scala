...
object Record extends SiteController {
  ...
  private[controllers] def check(i : models.Record.Id, p : Permission.Value = Permission.VIEW)(act : Record => SiteRequest[AnyContent] => Result) = SiteAction {
    implicit request =>
      models.Record.get(i).fold(NotFound : Result) {
        record =>
  ...
  def view(v : models.Volume.Id, i : models.Record.Id) = check(i) {
    record => implicit request =>
  ...
  def edit(v : models.Volume.Id, i : models.Record.Id) = check(i, Permission.EDIT) {
    record => implicit request =>
      val (m, f) = editFormFill(record)
      Ok(views.html.record.edit(record, m, f))
  }

  def update(v : models.Volume.Id, i : models.Record.Id) = check(i, Permission.EDIT) {
    record => implicit request =>
      ...
      form => BadRequest(views.html.record.edit(record, meas, form)), {
        case (category, data, (metric, datum)) =>
          ...
          def update(metric : Metric.Id, datum : Option[String]) : Option[String] =
            Metric.get(metric).fold(Some("measure.unknown") : Option[String]) {
              m =>
                datum.fold {
  ...
  def selectList(target : Slot)(implicit request : SiteRequest[_]) : Seq[(String, String)] = {
    ...
    target.volume.allRecords() map {
      r : Record =>
        (r.id.unId.toString, r.category.fold("")(_.name + ':') + r.ident.getOrElse("[" + r.id.unId.toString + "]"))
    }
  }

  def slotRemove(v : models.Volume.Id, s : models.Slot.Id, r : models.Record.Id, editRedirect : Boolean = false) = Slot.check(s, Permission.EDIT) {
    slot => implicit request =>
      slot.removeRecord(r)
      if (editRedirect)
  ...
  def slotAdd(v : models.Volume.Id, s : models.Slot.Id, editRedirect : Boolean = false) = Slot.check(s, Permission.EDIT) {
    slot => implicit request =>
      ...
      form.fold(
        ...
        _.fold({
          ...
          Created(views.html.record.edit(r, Nil, editForm)) : Result
            ...
            fold(BadRequest(Slot.viewEdit(slot)(recordForm = form.withError("record", "record.bad"))) : Result) {
          r =>
            r.addSlot(slot)
            if (editRedirect)
              Redirect(routes.Slot.edit(slot.volume.id, slot.id))
