...
object Entity extends SiteController {
  ...
  def authorizeSearch(i : Int, which : Boolean) = checkAdmin(i) { entity => implicit request =>
    ...
    form.fold(
      ...
      name => {
        val me = entity.id
        val res = models.Identity.byName(name).filter(e => e.id =!= me && e.id.notIn(Authorize.byParent(me).map(_.child)) && e.id.notIn(Authorize.byChild(me).map(_.parent))).take(8).list
