...
/** An action that is created by pre-processing the RequestHeader alone first (with an empty body) using an ActionBuilder[R] to produce a R for input into the main action body.
  ...
  */
final class DeferredAction[R[_]](builder : ActionBuilder[R], block : R[Unit] => Iteratee[Array[Byte], Result]) extends EssentialAction {
  private class DeferralResult(val request : R[Unit], header : ResponseHeader = ResponseHeader(0)) extends Result(header, Enumerator()) {
