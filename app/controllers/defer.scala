...
final case class EmptyRequest(request : RequestHeader) extends Request[Unit] {
  def body = ()
  def id = request.id
...
/** An action that is created by pre-processing the RequestHeader alone first (with an empty body) using an ActionBuilder[R] to produce a R for input into the main action body.
  ...
  */
sealed class DeferredAction[R[_]](builder : ActionBuilder[R], block : R[Unit] => Iteratee[Array[Byte], Result]) extends EssentialAction {
  protected class DeferralResult(val request : R[Unit], header : ResponseHeader = ResponseHeader(0)) extends Result(header, Enumerator()) {
  ...
  def apply(request : RequestHeader) : Iteratee[Array[Byte], Result] = Iteratee.flatten {
    builder.invokeBlock[Unit](EmptyRequest(request), r => Future.successful(new DeferralResult(r)))
    .map {
...
final class DeferredSiteAction[R[_] <: SiteRequest[_]](builder : ActionBuilder[R], block : R[Unit] => Iteratee[Array[Byte], Result])
...
object DeferredAction {
  ...
  def apply[R[_]](builder : ActionBuilder[R])(block : R[Unit] => Iteratee[Array[Byte], Result]) : EssentialAction =
    new DeferredAction(builder, block)
  def site[R[_] <: SiteRequest[_]](builder : ActionBuilder[R])(block : R[Unit] => Iteratee[Array[Byte], Result]) : EssentialAction =
