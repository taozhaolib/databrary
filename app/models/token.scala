...
/** A token, usually sent via email, granting automatic login or registration to the given party.
  ...
  */
final class LoginToken protected (token : String, expires : Timestamp, val party : Party, val password : Boolean) extends Token(token, expires) {
...
object LoginToken extends TokenTable[LoginToken]("login_token") {
  private def make(party : Party)(token : String, expires : Timestamp, password : Boolean) =
    new LoginToken(token, expires, party, password)
  private val columns = Columns[
    String, Timestamp, Boolean](
    'token, 'expires,  'password)
  private[models] val row = columns.join(Party.row, "login_token.party = party.id").
  ...
  /** Issue a new token for the given party.
    ...
    */
  def create(party : Party, password : Boolean = false)(implicit db : Site.DB) : LoginToken = {
    if (password)
      SQL("DELETE FROM login_token WHERE party = {party} AND password").on('party -> party.id).execute
    val args = SQLArgs('party -> party.id, 'password -> password)
    SQL("INSERT INTO login_token " + args.insert + " RETURNING " + columns.select)
      .single(columns.map(make(party) _))
  }
}
