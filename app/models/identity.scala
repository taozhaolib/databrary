...
class Identity(private val entity : Entity) extends TableRow {
  ...
  final def authorizeParents(all : Boolean = false)(implicit db : Site.DB) = Authorize.getParents(this.id, all)
  final def authorizeChildren(all : Boolean = false)(implicit db : Site.DB) = Authorize.getChildren(this.id, all)

  final def delegated(implicit site : Site) = Authorize.delegate_check(site.identity.id, this.id)(site.db)
  final def delegatedBy(p : Identity.Id)(implicit site : Site) = Authorize.delegate_check(this.id, p)(site.db)
  final def studyAccess(p : Permission.Value)(implicit site : Site) = StudyAccess.getStudies(this.id, p)
}
