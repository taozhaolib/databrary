...
object Identity extends TableViewId[Identity]("entity LEFT JOIN account USING (id)", _.id) {
  ...
  def get(i : Int)(implicit db : Site.DB) : Identity =
    IdentityCache.getOrElseUpdate(i, 
      SQL("SELECT * FROM " + table + " WHERE id = {id}").
        on('id -> Some(i)).single(row))
  ...
  def searchForAuthorize(name : String, who : Identity)(implicit db : Site.DB) =
  ...
  def searchForStudyAccess(name : String, study : Study)(implicit db : Site.DB) =
...
object User extends TableViewId[User]("entity JOIN account USING (id)", _.id) {
  ...
  def get(i : Int)(implicit db : Site.DB) : Option[User] = 
    Identity.get(i).user
